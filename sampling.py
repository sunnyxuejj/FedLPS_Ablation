...
def cifar_extr_noniid(train_dataset, test_dataset, num_users, n_class, num_samples, rate_unbalance):
    ...
    for i in range(num_users):
        ...
        for label in user_labels_set:
            select_test = idxs_test[int(label) * num_imgs_perc_test:int(label + 1) * num_imgs_perc_test]
            select_test = np.random.choice(select_test, num_samples, replace=False)
            dict_users_test[i] = np.concatenate((dict_users_test[i], select_test), axis=0)
...
def cifar100_extr_noniid(train_dataset, test_dataset, num_users, n_class, num_samples, rate_unbalance):
    num_shards_train, num_imgs_train = int(50000/num_samples), num_samples
    num_classes = 100
...
def tiny_extr_noniid(train_dataset, test_dataset, num_users, n_class, num_samples, rate_unbalance):
    num_shards_train, num_imgs_train = int(100000/num_samples), num_samples
    num_classes = 200
    num_imgs_perc_test, num_imgs_test_total = 50, 10000
    assert(n_class * num_users <= num_shards_train)
    assert(n_class <= num_classes)
    idx_class = [i for i in range(num_classes)]
    idx_shard = [i for i in range(num_shards_train)]
